...
from modules import Highway, RNNEncoder, SimpleSoftmaxLayer, BasicAttn, BiDAFAttn, Modeling, RNNDecoder
...
#        self.batch_generator_fn = batch_generator_fn
        ...
        learning_rate = tf.train.exponential_decay(learning_rate=FLAGS.learning_rate, global_step=self.global_step, decay_steps=1, decay_rate=0.9999)
        opt = tf.train.AdamOptimizer(learning_rate) # you can try other optimizers
    ...
    def add_dataset(self):
        ...
        self.dataset = tf.data.Dataset().from_generator(batch_generator_fn, output_types)
        self.dataset.repeat().prefetch(512)
        self.datasest = self.dataset.make_one_shot_iterator()
    ...
    def add_placeholders(self):
        ...
        """
        self.context_ids, self.conntext_mask, self.qn_ids, self.qn_mask, self.ans_span = self.dataset.get_next()
        self.keep_prob = 1.0 - self.FLAGS.dopout
        """
    def add_embedding_layer(self, emb_matrix):
        ...
        with vs.variable_scope("embeddings"):
            ...
            m = np.load('../data/character_embeddings.npy').item()
            char_embedding_matrix = tf.constant(m, dtype=tf.float32, name="char_emb_matrix")
            print(char_embedding_matrix.shape)
            self.char_context_embs = embedding_ops.embedding_lookup(char_embedding_matrix, self.context_ids) # shape (batch_size, context_len, embedding_size)
            self.char_qn_embs = embedding_ops.embedding_lookup(char_embedding_matrix, self.qn_ids) # shape (batch_size, question_len, embedding_size)
            '''
    ...
    def build_graph(self):
        ...
        self.context_embs = highway_cn.build_graph(self.context_embs) # we do not use masking because the output is bassed to an encoder that applies masking itself
        ...
        # Use context hidden states to attend to question hidden states
        attn_layer = BiDAFAttn(self.FLAGS.hidden_size, self.FLAGS.context_len, self.FLAGS.question_len)
        attn_output = attn_layer.build_graph(context_hiddens, question_hiddens, self.context_mask, self.qn_mask) # attn_output is shape (batch_size, context_len, hidden_size*2)

        # Modeling layer
        modeling_layer = Modeling(self.FLAGS.hidden_size)
        modeling_output = modeling_layer.build_graph(attn_output, self.context_mask)

        # RNNDecoder
        decoder = RNNDecoder(self.FLAGS.hidden_size)
        start_features, end_features = decoder.build_graph(modeling_output, attn_output, self.context_mask)

        with vs.variable_scope("StartDist"):
            softmax_layer_start = SimpleSoftmaxLayer()
            self.logits_start, self.probdist_start = softmax_layer_start.build_graph(start_features, self.context_mask)

        with vs.variable_scope("EndDist"):
            softmax_layer_end = SimpleSoftmaxLayer()
            self.logits_end, self.probdist_end = softmax_layer_end.build_graph(end_features, self.context_mask)
